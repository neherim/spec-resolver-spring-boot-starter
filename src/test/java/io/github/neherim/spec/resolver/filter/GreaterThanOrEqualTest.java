...
public class GreaterThanOrEqualTest extends BaseContextTest {

    public static final String SEARCH_URL = "/greater-than-or-equal";
    @Autowired
    TestRepo testRepo;
    ...
    void findByIntField() throws Exception {
        mockMvc.perform(get(SEARCH_URL + "?idFrom=2"))
    ...
    void findByDateField() throws Exception {
        mockMvc.perform(get(SEARCH_URL + "?dateFrom=2023-12-12"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
    ...
    public static class TestEntity {
        @Id
        @Filter(name = "idFrom", filter = GreaterThanOrEqual.class)
        public Integer id;
        @Filter(name = "dateFrom", filter = GreaterThanOrEqual.class)
